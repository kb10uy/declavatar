use crate::decl::{
    compiler::{deconstruct_node, DeclError, DeclErrorKind, NodeEntries, Result},
...
pub fn compile_drivers(node: &KdlNode) -> Result<Drivers> {
    ...
    for child in children {
        ...
        let group = match child_name {
            NODE_NAME_GROUP => compile_driver_group(child)?,
            _ => {
...
fn compile_driver_group(node: &KdlNode) -> Result<DriverGroup> {
...
fn compile_drive(node: &KdlNode) -> Result<Drive> {
